...
- **Chrome.** Hairline borders over low-fill surfaces, square-ish corners, grey uppercase section headers + white body, generous horizontal padding with compact rows. A restrained **Indigo accent** (`--accent: #6366f1`, `--accent-soft`) is used for <10% of the surface: the active-nav "you are here" dot and the focus ring only — never a fill or a side stripe.
