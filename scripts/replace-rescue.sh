...
echo "==> Drift policy: rescue user-edited files to personal/; route .agents|.codex|.obsidian|MIGRATION.md user-edits to .hq-conflicts/rescue-$RUN_TS/; leave user-only files untouched; drop master-sync symlinks (regenerated by master-sync.sh)"
...
process_one() {
  ...
  if [ "$user_edited" = "1" ]; then
    if [ "$DRY_RUN" = "1" ]; then
      ...
      if [ "$rel" != ".claude/CLAUDE.md" ] && is_conflict_class "$rel"; then
        conflict_one "$rel"
      else
        rescue_one "$rel"
      fi
...
echo ""
echo "==> Walk complete (user-only: $COUNT_USER_ONLY, unchanged-deleted: $COUNT_UNCHANGED, user-edit-rescued: $COUNT_USER_EDIT, conflict-quarantined: $COUNT_USER_CONFLICT, symlinks-dropped: $COUNT_SYMLINK_DROPPED)"
