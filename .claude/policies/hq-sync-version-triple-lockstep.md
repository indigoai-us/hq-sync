...
Four version locations must always agree before a release tag is cut:
...
4. `src-tauri/Cargo.lock` — the `hq-sync-menubar` package entry (regenerated by `cargo`, but must be committed in lockstep so a clean CI checkout resolves the same version)

The git tag must match the Tauri bundle version: `git tag vX.Y.Z` ⇔ `tauri.conf.json` reports `X.Y.Z`. CI fails the release if the pushed tag version ≠ `tauri.conf.json` version.
...
grep -A1 'name = "hq-sync-menubar"' src-tauri/Cargo.lock | grep '^version'
```

All four must print the same `X.Y.Z[-channel.N]`, and the tag you are about to push must be `vX.Y.Z[-channel.N]`.
