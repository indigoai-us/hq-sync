<script lang="ts">
  ...
  // Sync / Meetings / Library / Messages are the four top-level destinations;
  // everything after them is a company row.
  const primaryRows = $derived(rows.slice(0, 4));
  const companyRows = $derived(rows.slice(4));
</script>
